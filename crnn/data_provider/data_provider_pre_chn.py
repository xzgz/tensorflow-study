...
try:
    ...
    pass
from random import shuffle as rd_shf
...
class TextDataProvider(object):
    ...
    def __init__(self, dataset_dir, annotation_name, validation_set=None, validation_split=None, shuffle=None,
        ...
        train_anno_path = ops.join(self.__dataset_dir, annotation_name)
        ...
        with open(train_anno_path, 'r') as anno_file:
            ...
            train_images = []
            train_image_widths = []
            for index, imna in enumerate(imagenames):
                imname = imna
                im = cv2.imread(imname, cv2.IMREAD_COLOR)
                if index % 2 == 0:
                    im_new_tmp = im.transpose(1, 0, 2)
                    im_new = np.zeros_like(im_new_tmp)
                    for k in range(im_new.shape[0]):
                        im_new[k, :] = im_new_tmp[-(k+1), :]
                    image = im_new
                else:
                    image = im
                ...
                if new_w <= 48:
                    ...
                    train_images.append(image)
                    train_image_widths.append(int(48/4))
                assert image.shape[1] == 48
                if index % (10*10000) == 0:
                    print('Read %d images.' % index)
            print('Read complete.')
            train_labels = labels
            train_imagenames = [ops.basename(tmp) for tmp in imagenames]
