...
class Dataset(object):
    ...
    @staticmethod
    def shuffle_images_labels(images, labels, imagenames, image_widths):
        """
        ...
        assert len(images) == len(image_widths)
        random_index = np.random.permutation(len(images))
        shuffled_images = []
        shuffled_labels = []
        shuffled_imagenames = []
        shuffled_image_widths = []
        for i in random_index:
            shuffled_images.append(images[i])
            shuffled_labels.append(labels[i])
            shuffled_imagenames.append(imagenames[i])
            shuffled_image_widths.append(image_widths[i])
        # shuffled_images = images[random_index]
        # shuffled_labels = labels[random_index]
        # shuffled_imagenames = imagenames[random_index]
        # shuffled_image_widths = image_widths[random_index]

        return shuffled_images, shuffled_labels, shuffled_imagenames, shuffled_image_widths
