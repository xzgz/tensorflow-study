...
class ShadowNet(cnn_basenet.CNNBaseModel):
    ...
    def __feature_sequence_extraction(self, inputdata):
        ...
        :param inputdata: eg. batch*h*w*3 NHWC format
        :return relu7: batch*h/32*w/4*512
        """
        # batch*h/2*w/2*64
        ...
        if self.phase.lower() == 'train':
            bn3 = self.layerbn(inputdata=conv3, is_training=True)
        else:
            bn3 = self.layerbn(inputdata=conv3, is_training=False)
        relu3 = self.relu(bn3)
        # batch*h/4*w/4*256
        conv4 = self.conv2d(inputdata=relu3, out_channel=256, kernel_size=3, stride=1, use_bias=False, name='conv4')
        relu4 = self.relu(conv4)
        # batch*h/8*w/4*256
        max_pool4 = self.maxpooling(inputdata=relu4, kernel_size=[2, 1], stride=[2, 1], padding='VALID')
        # batch*h/8*w/4*512
        conv5 = self.conv2d(inputdata=max_pool4, out_channel=512, kernel_size=3, stride=1, use_bias=False, name='conv5')
        if self.phase.lower() == 'train':
            bn5 = self.layerbn(inputdata=conv5, is_training=True)
        else:
            bn5 = self.layerbn(inputdata=conv5, is_training=False)
        relu5 = self.relu(bn5)
        # batch*h/8*w/4*512
        conv6 = self.conv2d(inputdata=relu5, out_channel=512, kernel_size=3, stride=1, use_bias=False, name='conv6')
        relu6 = self.relu(conv6)
        # batch*h/16*w/4*512
        max_pool6 = self.maxpooling(inputdata=relu6, kernel_size=[2, 1], stride=[2, 1])
        # batch*h/32*w/4*512
        conv7 = self.conv2d(inputdata=max_pool6, out_channel=512, kernel_size=2, stride=[2, 1], use_bias=False, name='conv7')
        if self.phase.lower() == 'train':
            bn7 = self.layerbn(inputdata=conv7, is_training=True)
        else:
            bn7 = self.layerbn(inputdata=conv7, is_training=False)
        relu7 = self.relu(bn7)
        return relu7
    ...
    def __sequence_label(self, inputdata):
        ...
        with tf.variable_scope('LSTMLayers'):
            ...
            stack_lstm_layer, _, _ = rnn.stack_bidirectional_dynamic_rnn(
                fw_cell_list, bw_cell_list, inputdata, sequence_length=self.__seq_length, dtype=tf.float32)
            ...
            print('inputdata shape:', inputdata.get_shape().as_list())
            print('stack_bidirectional_dynamic_rnn output shape:', stack_lstm_layer.get_shape().as_list())
